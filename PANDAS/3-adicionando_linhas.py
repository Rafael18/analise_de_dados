...
forma_pgmt_selecionado = st.sidebar.selectbox("Forma de Pagamento", ["cartão de crédito", "boleto", "pix", "dinheiro"])
...
if st.sidebar.button("Adicionar Nova compra"):
    lista_adicionar = [df_compras['id_compra'].max() + 1 if not df_compras.empty else 1,
                        ...
                        nome_cliente,
                        genero_selecionado,
                        forma_pgmt_selecionado 
                      ]
    # st.write(lista_adicionar)
